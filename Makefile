...
LD_FLAGS = -X github.com/pylons-tech/juno.Version=$(VERSION) \
	-X github.com/pylons-tech/juno.Commit=$(COMMIT)
...
format:
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -name '*.pb.go' | xargs goimports -w -local github.com/pylons-tech/juno
.PHONY: format
